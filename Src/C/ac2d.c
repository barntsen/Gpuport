  ...
  struct ac2d *Ac2dNew(struct model *Model)
{
  ...
  Ac2d = (struct ac2d*)malloc(sizeof(struct ac2d)); 
  Ac2d->p=(float*)malloc(sizeof(float)*Nx*Ny);
  Ac2d->vx=(float*)malloc(sizeof(float)*Nx*Ny);
  Ac2d->vy=(float*)malloc(sizeof(float)*Nx*Ny);
  Ac2d->exx=(float*)malloc(sizeof(float)*Nx*Ny);
  Ac2d->eyy=(float*)malloc(sizeof(float)*Nx*Ny);
  Ac2d->gammax=(float*)malloc(sizeof(float)*Nx*Ny);
  Ac2d->gammay=(float*)malloc(sizeof(float)*Nx*Ny);
  Ac2d->thetax=(float*)malloc(sizeof(float)*Nx*Ny);
  Ac2d->thetay=(float*)malloc(sizeof(float)*Nx*Ny);
...
int Ac2dvx(struct ac2d *Ac2d, struct model *Model)
{
  ...
  for(j=0; j<ny; j=j+1)
  {
    for(i=0; i<nx; i=i+1)
    {
        Ac2d->vx[idx2(nx,i,j)] = Model->Dt*(Model->Rho[idx2(nx,i,j)])*Ac2d->exx[idx2(nx,i,j)] 
                               + Ac2d->vx[idx2(nx,i,j)]
                               + Model->Dt*Ac2d->thetax[idx2(nx,i,j)]*Model->Drhox[idx2(nx,i,j)];
...
int Ac2dvy(struct ac2d *Ac2d, struct model *Model)
{
  ...
  for(j=0; j<ny; j=j+1)
  {
    for(i=0; i<nx; i=i+1)
    {
    Ac2d->vy[idx2(nx,i,j)] = Model->Dt*(Model->Rho[idx2(nx,i,j)])*Ac2d->eyy[idx2(nx,i,j)] 
                           + Ac2d->vy[idx2(nx,i,j)]
                           + Model->Dt*Ac2d->thetay[idx2(nx,i,j)]*Model->Drhoy[idx2(nx,i,j)];
...
int Ac2dstress(struct ac2d *Ac2d, struct model *Model)
{
  ...
  for(j=0; j<ny; j=j+1)
  {
    for(i=0; i<nx; i=i+1)
    {
      Ac2d->p[idx2(nx,i,j)] = Model->Dt*Model->Kappa[idx2(nx,i,j)]
                             *(Ac2d->exx[idx2(nx,i,j)] + Ac2d->eyy[idx2(nx,i,j)]) 
                             + Ac2d->p[idx2(nx,i,j)]
                             + Model->Dt*(Ac2d->gammax[idx2(nx,i,j)]*Model->Dkappax[idx2(nx,i,j)]
